...
public class HeaderPositionCalculator {
    ...
    /**
     * The following fields are used as buffers for internal calculations. Their sole purpose is to avoid allocating new Rect every
     * time we need one.
     */
    ...
    /**
     * Determines if a view should have a sticky header. The view has a sticky header if: 1. It is the first element in the recycler
     * view 2. It has a valid ID associated to its position
     *
    ...
    /**
     * Determines if an item in the list should have a header that is different than the item in the list that immediately precedes
     * it. Items with no headers will always return false.
     *
        ...
        if (firstViewUnderHeaderPosition > 0 && hasNewHeader(firstViewUnderHeaderPosition, isReverseLayout)) {
            ...
            if (mOrientationProvider.getOrientation(recyclerView) == LinearLayoutManager.VERTICAL) {
                ...
                int bottomOfThisHeader =
                  recyclerView.getPaddingTop() + stickyHeader.getBottom() + mTempRect2.top + mTempRect2.bottom;
                ...
                int rightOfThisHeader =
                  recyclerView.getPaddingLeft() + stickyHeader.getRight() + mTempRect2.left + mTempRect2.right;
    ...
    private void translateHeaderWithNextHeader(RecyclerView recyclerView, int orientation, Rect translation,
      View currentHeader, View viewAfterNextHeader, View nextHeader) {
        ...
        if (orientation == LinearLayoutManager.VERTICAL) {
            ...
            int shiftFromNextHeader = viewAfterNextHeader.getTop() - nextHeader.getHeight() - mTempRect1.bottom - mTempRect1.top -
              currentHeader.getHeight() - topOfStickyHeader;
            ...
            int shiftFromNextHeader = viewAfterNextHeader.getLeft() - nextHeader.getWidth() - mTempRect1.right - mTempRect1.left -
              currentHeader.getWidth() - leftOfStickyHeader;
